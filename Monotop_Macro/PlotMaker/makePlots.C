 ...
 bool dology = false;
  ...
  for(int iselstep=0; iselstep < selectionStep_list.size(); iselstep++){
    ...
    for(int ivar=0; ivar < variables_list.size(); ivar++){
	  ...
	  PlotStack(variables_list[ivar],  channel_list[ichan],  selectionStep_list[iselstep] , dology, dataSample_list,  channel_list, mcSample_list, signalSample_list, colorVector, dataDrivenTemplates_list, QCDCorr, false);
      }
