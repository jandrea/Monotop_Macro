...
float getQuadraticHistoError(TH1D* histo)
{
    TH1D* tmp = (TH1D*) histo->Clone();
    float sum_err =0;

    for(unsigned short int i = 1; i <tmp->GetNbinsX(); i++)
    {
        sum_err += pow( tmp->GetBinError(i), 2);
        //cout << "Bin[" << i << "] = " << tmp->GetBinContent(i) << " | err = " << tmp->GetBinError(i) << endl;
    }
    //cout << "Integral= " << histo->Integral() << " | Sum_err = " << sum_err << endl;

    sum_err = pow(sum_err, 0.5);
...
void CutFlow(TString signal_type){
  ...
  if(signal_type == "FCNC")
  {
    samplelist_signal.push_back("S4Inv100");  samplename_signal.push_back("\\bm{$S4(100)$}");
    samplelist_signal.push_back("S4Inv500");  samplename_signal.push_back("\\bm{$S4(500)$}");
    samplelist_signal.push_back("S4Inv1000"); samplename_signal.push_back("\\bm{$S4(1000)$}");
  }
  if(signal_type == "RES")
  {
  ...
  TFile *f_signal  = new TFile("../inputTheta_merged_AllSignals_AllRegions_2j2b.root");
  f_signal->cd();
  ...
  string ofilenametex = "CutFlow_postFit_2j2b.tex";
  ...
  for(unsigned int iback = 0; iback < samplelist_MC.size(); iback++)
  {
    ...
    for(unsigned int ireg = 0; ireg < regions.size(); ireg++)
    {
        ...
        if(iback < 7)
        {
            ...
            {
                ofile << simplifyWriting(tmp->Integral()) ;
                ofile << "$\\pm$" << simplifyWriting(getQuadraticHistoError(tmp)) ;
            }
        ...
        else if(iback == 7 || iback == 8)
        {
            if(ireg == 0)
            {
                total_SingleTop_0       += tmp->Integral();
                total_SingleTop_error_0 += pow(getQuadraticHistoError(tmp), 2);
                if(iback == 8) ofile << simplifyWriting(total_SingleTop_0) << "$\\pm$" << simplifyWriting(pow(total_SingleTop_error_0, 0.5)) << " & " ;
            }
            else if(ireg == 1)
            {
                total_SingleTop_1       += tmp->Integral();
                total_SingleTop_error_1 += pow(getQuadraticHistoError(tmp), 2);
                if(iback == 8) ofile << simplifyWriting(total_SingleTop_1) << "$\\pm$" << simplifyWriting(pow(total_SingleTop_error_1, 0.5)) << " & " ;
            }
            else if(ireg == 2)
            {
                total_SingleTop_2       += tmp->Integral();
                total_SingleTop_error_2 += pow(getQuadraticHistoError(tmp), 2);
	    ...
	    if(ireg == 0)
        {
            total_background_0        +=  tmp->Integral();
	        total_background_error_0  +=  pow(getQuadraticHistoError(tmp), 2);
        }
        else if(ireg == 1)
        {
            total_background_1        +=  tmp->Integral();
	        total_background_error_1  +=  pow(getQuadraticHistoError(tmp), 2);
        }
        else if(ireg == 2)
        {
            total_background_2        +=  tmp->Integral();
	        total_background_error_2  +=  pow(getQuadraticHistoError(tmp), 2);
        }

       // cout << "sample = " << samplelist_MC[iback] << " | and Yield =" << tmp->Integral() <<  " | err = " << getQuadraticHistoError(tmp) << endl;
